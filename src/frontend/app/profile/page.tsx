...
import { getIssuesByUserId, getUserById } from '@/backend/server/data';
import { IssueCard } from '@/frontend/components/issues/IssueCard';
import { Card, CardContent, CardHeader, CardTitle } from '@/frontend/components/ui/card';
import { Avatar, AvatarFallback, AvatarImage } from '@/frontend/components/ui/avatar';
import { ListChecks, CheckSquare, Hourglass, Award, Gift } from 'lucide-react';

const MOCKED_USER_ID = 'user-1';

const POINTS_PER_RESOLUTION = 50;
const POINTS_TO_MATIC_RATE = 150 / 10; // 150 points = 10 MATIC

export default async function ProfilePage() {
  const user = await getUserById(MOCKED_USER_ID);
  const userIssues = await getIssuesByUserId(MOCKED_USER_ID);
