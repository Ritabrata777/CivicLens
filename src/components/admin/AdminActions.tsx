    ...
    const handleUpdateStatus = (newStatus: IssueStatus, notes?: string) => {
        startTransition(async () => {
            if (newStatus === 'Accepted') {
                try {
                    const { connectWallet, verifyIssueOnChain } = await import("@/lib/web3");
                    const walletAddress = await connectWallet();
                    if (!walletAddress) {
                        toast({ title: "Wallet Error", description: "Please connect wallet to verify on blockchain.", variant: "destructive" });
                        return;
                    }

                    toast({ title: "Blockchain Verification", description: "Please confirm transaction in MetaMask..." });
                    // Using wallet address as adminId for on-chain record
                    await verifyIssueOnChain(issueId, walletAddress, newStatus);
                    toast({ title: "Blockchain Success", description: "Issue verified on chain!" });

                } catch (e) {
                    console.error(e);
                    toast({ title: "Blockchain Error", description: "Verification failed. Database not updated.", variant: "destructive" });
                    return;
                }
            }
